...
- [x] Write CLAUDE.md with the spec Section 2 rules verbatim plus toolchain lines.
- [x] Write `docs/datasets.md` with rows: Pexels 39221979 (Pexels licence, 1920×1080, 35 s), Pexels 854634 (CC0, 1280×720, 18 s), yolo26n.pt and rtdetr-l.pt (ultralytics/assets v8.4.0, AGPL-3.0), SH17 (CC BY-NC-SA 4.0, Kaggle URL, manual), MVTec-AD (CC BY-NC-SA 4.0, manual), MOT17 (manual, not required).
- [x] `uv sync` in both `edge/` and `cloud/`; verify `python -c "import torch; print(torch.backends.mps.is_available())"` prints True.
- [x] Commit: `chore: scaffold edge and cloud packages, CLAUDE.md, dataset ledger`.
...
```
- [x] Tests: `FrameSource(file, loop=True)` yields more frames than the clip has (loops); `blur_faces` changes pixels inside the head region and nowhere else; `shelf_empty_ratio` on a flat grey image > 0.8 and on a busy fixture < 0.4; `classify_machine_state` returns `stopped` for identical ROIs and `running` for noisy differing ROIs.
- [x] Commit `feat(edge): capture with face blur, shelf and machine-state estimators`.
...
```
- [x] Tests: append/unsynced/mark_synced round trip; `ClipWriter` produces an mp4 with duration between 8 and 12 s; `Syncer.push_once` against `httpx.MockTransport` returning 503 leaves events unsynced, then 200 marks them synced; batch payload uses `Event.to_dict()`.
- [x] Commit `feat(edge): sqlite store, clip ring buffer, offline sync`.
...
- [x] Test: `run_pipeline(site, max_frames=60, detector="yolo", api=None)` on the checkout clip produces ≥1 `footfall_tick` or `checkout_served` event in SQLite and reports fps in the returned stats.
- [x] Manual check: `--preview` shows boxes, zones, and blurred heads at ≥ 15 fps on M4 Pro; record the fps in README.
- [x] Commit `feat(edge): pipeline, CLI, preview window`.
...
- [x] Test: missing dataset exits with code 2 and message containing "CC BY-NC-SA".
- [x] Commit `feat(edge): training scripts and eval report`.
...
- [x] Tests (TestClient + sqlite tmp): batch post of 3 events returns 201 with `inserted=3`, repost returns `inserted=0`; filter by severity; get by id 404 for unknown; `/stream` yields the event posted after subscription (use anyio task).
- [x] Commit `feat(api): models, events, clips, SSE stream`.
...
```
- [x] Tests: textbook M/M/2 with λ=4, μ=3 gives ρ=0.667, Lq≈1.067, Wq≈0.267 h (tolerance 1e-3); c=1 reduces to M/M/1 formulas; ρ≥1 returns `Wq=math.inf`; service level on synthetic events; OSA = 1 − empty time fraction.
- [x] Commit `feat(api): M/M/c queue model, service level, OSA KPIs`.
...
```
- [x] Tests: on a synthetic 21-day series with daily+weekly seasonality plus noise, GBR MAE < naive MAE; < 14 days returns `sufficient=False`; toy 3-slot MILP picks minimum tills keeping ρ ≤ 0.85 (hand-computed: λ=[2,5,8], μ=3 → tills [1,2,4]).
- [x] Commit `feat(api): GBR forecast vs seasonal naive, staffing MILP`.
...
- [x] Tests: report renders in all three languages with no empty sections; exactly three recommendations; before/after simulation reduces total W_q when proposals applied.
- [x] Commit `feat(api): weekly bilingual report and before/after simulation`.
...
- [x] Tests: seed produces events for every kind of the chosen profile; all have `simulated=true`; deterministic with seed.
- [x] Commit `feat(api): labelled history simulator for demo and forecasting`.
...
- [x] Tests (vitest): `severityMeta(3)` returns label, icon name, token `--critical`; every key in `en.json` exists in `hi.json` and `ar.json`; `ar` locale sets `dir="rtl"`.
- [x] Verify in browser: theme toggle persists, RTL flips the rail, fonts load.
- [x] Commit `feat(web): control-room shell, tokens, theme toggle, EN/HI/AR`.
...
- [x] Tests: binning puts an event at 00:07 into bin 1, 23:59 into bin 287; ticks preserve ids.
- [x] Commit `feat(web): 24-hour event tape`.
...
- [x] Test: `useEventStream` reducer prepends new events and caps at 200.
- [x] Commit `feat(web): dashboard with KPIs, live stream, queue model`.
...
- [x] Verify 60 fps in Chrome performance panel on the desktop; mobile viewport renders.
- [x] Commit `feat(web): R3F 3D floor with live pulses`.
...
- [x] Tests: `ProposalCard` renders Approve/Reject and calls the API; approving shows "Approved".
- [x] Commit `feat(web): event detail, actions queue, shelves, forecast, report`.
...
- [x] Commit `test(web): playwright smoke flow`.
...
- [x] Verify `scripts/dev.sh` brings all three up and a live edge event appears in the browser within 30 s.
- [x] Commit `chore: dev script, docker, render and vercel configs`.
...
- [x] Gate: `grep -rE "TODO|XX|\[insert" docs/` returns nothing; every number in the report maps to a key in `docs/results/`.
- [x] Commit `docs: Term 4 report, deck, notebook, viva, SOP, README`.
