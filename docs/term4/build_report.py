...
def main() -> None:
    ...
    h(doc, "Appendix")
    para(doc, "A. Site configuration: edge/sites/retail_demo.yaml (zones in normalised coordinates, thresholds, floor plan in metres). B. Rule table: Table 1 and edge/raqib_edge/rules.py. C. Tool schemas: cloud/raqib_api/agent/tools.py. D. Test coverage: edge 47 tests (rules with synthetic tracks, detector adapter on real frames, blur, store, offline sync, end-to-end pipeline); cloud 41 tests (routers, Erlang-C textbook cases, policies, tool validation, forecast beats naive, MILP toy problem, report in three languages); web 15 unit tests plus a Playwright smoke flow (event → clip → approve → executed). E. Results: docs/results/*.json and docs/results/figures/. F. Generation: docs/term4/build_report.py.", size=9.5)
