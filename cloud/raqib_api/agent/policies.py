...
class Policy:
    ...
    def check(self, event: Event, proposed: list[Call], confidence: float, lang: str = "en") -> PolicyOutcome:
        ...
        for call in proposed:
            ...
            if confidence < REVIEW_CONFIDENCE and call.tool in AUTONOMOUS_TOOLS - {"escalate", "send_alert", "request_human_review"}:
                out.dropped.append((call, f"P5: confidence {confidence:.2f} < {REVIEW_CONFIDENCE}"))
                continue
        ...
        if confidence < REVIEW_CONFIDENCE and "request_human_review" not in seen_tools:
