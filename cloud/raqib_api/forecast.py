...
def fit_predict(series: pd.Series, horizon: int = 24, holdout_h: int = 24 * 7) -> ForecastResult:
    ...
    df = _features(series).dropna()
    if len(df) <= holdout_h + 24:
        return ForecastResult(sufficient=False, reason="not enough rows after feature lags")
    train, test = df.iloc[:-holdout_h], df.iloc[-holdout_h:]
    model = GradientBoostingRegressor(n_estimators=300, max_depth=3, learning_rate=0.03, subsample=0.9,
    ...
    for t in fut_idx:
        ...
        f = _features(ext).iloc[[-1]][FEATURES].ffill(axis=1).fillna(0.0)
    ...
    return ForecastResult(True, "", history, forecast, round(mae, 3), None if mape is None else round(mape, 1),
                          round(mae_naive, 3), round(improvement, 1), peaks, FEATURES)
